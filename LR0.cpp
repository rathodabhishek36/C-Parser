// Abhishek Rathod.
// Enroll. No. - 17114004
// This program takes as input production rules and start symbol and gives as output set of LR(0) items.
...
void print_lr0_items(){
	...
	for(int i=0;i<lr0_items.size();i++){
		cout<<"--------------------------"<<endl ;
		cout<<"I"<<i<<" :"<<endl ;
		for(int j=0;j<lr0_items[i].size();j++){
			cout<<lr0_items[i][j]<<endl ;
		}
	}
	cout<<"--------------------------"<<endl ;
}
